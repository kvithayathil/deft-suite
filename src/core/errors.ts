...
export function skillNotFound(
  ...
  return new SkillMcpError(
    ErrorCode.SkillNotFound,
    `Skill '${name}' not found in any configured source.`,
    'not_found',
    details,
    true,
    false,
...
export function scanFailed(skillName: string, findings: ScanFinding[]): SkillMcpError {
  return new SkillMcpError(
    ...
    `Security scan failed for skill '${skillName}'. ${findings.length} finding(s).`,
    'security',
    { findings, skillName },
    false,
    false,
...
export function networkUnavailable(url: string, reason: string): SkillMcpError {
  return new SkillMcpError(
    ...
    'network',
    { url, reason, suggestion: 'Check your network connection or use local/cached sources.' },
    true,
    true,
  );
}
...
export function accessDenied(skillName: string, sourceUrl: string, policy: string): SkillMcpError {
  return new SkillMcpError(
    ...
    `Skill '${skillName}' from '${sourceUrl}' is not permitted by your access control policy.`,
    'security',
    {
      skillName,
      sourceUrl,
      policy,
      suggestion: 'Contact your admin or update security.accessControl in your config.',
    },
    false,
    false,
  );
...
export function alreadyInstalled(skillName: string, currentVersion?: string): SkillMcpError {
  return new SkillMcpError(
    ...
    'conflict',
    {
      skillName,
      currentVersion,
      suggestion: 'Use --update flag to update the existing installation.',
    },
    true,
    false,
  );
...
export function skillQuarantined(skillName: string, findings: ScanFinding[]): SkillMcpError {
  return new SkillMcpError(
    ErrorCode.SkillQuarantined,
    `Skill '${skillName}' is quarantined due to security scan findings.`,
    'security',
    {
      skillName,
      findings,
      suggestion: 'Review findings via get_status, then fix or force-approve.',
    },
    true,
    false,
  );
...
export function resourceTooLarge(
  skillName: string,
  resourcePath: string,
  sizeBytes: number,
): SkillMcpError {
  return new SkillMcpError(
    ...
    `Resource '${resourcePath}' in skill '${skillName}' exceeds size limit (${sizeBytes} bytes).`,
    'validation',
    { skillName, resourcePath, sizeBytes },
    false,
    false,
  );
}
...
export function skillScanning(skillName: string): SkillMcpError {
  return new SkillMcpError(
    ErrorCode.SkillScanning,
    `Skill '${skillName}' is currently being scanned. Previous version served if available.`,
    'conflict',
    { skillName },
    true,
    true,
...
export function skillLocked(skillName: string): SkillMcpError {
  return new SkillMcpError(
    ErrorCode.SkillLocked,
    `Skill '${skillName}' is locked and cannot be removed.`,
    'conflict',
    { skillName },
    false,
    false,
...
export function configLocked(key: string): SkillMcpError {
  return new SkillMcpError(
    ...
    `Config key '${key}' is locked and cannot be modified at this level.`,
    'permission',
    { key },
    false,
    false,
...
export function alreadyExists(skillName: string): SkillMcpError {
  return new SkillMcpError(
    ...
    `Skill '${skillName}' already exists in the store.`,
    'conflict',
    { skillName, suggestion: 'Use a different name or update the existing skill.' },
    true,
    false,
  );
}
