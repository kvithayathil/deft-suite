---
name: usage-stats
description: Interpret skill usage and frecency trends using skill-mcp usage and stats commands
---
...
- `skill-mcp stats` — high-level dashboard for top skills, search activity, and usage database health
- `skill-mcp usage export --format json` — full raw usage data for deeper analysis
- `skill-mcp usage export --format csv` — spreadsheet-friendly export for reports
- `skill-mcp usage reset <name>` — reset one skill's accumulated score
- `skill-mcp usage reset --all` — reset all usage/frecency data
...
1. Run `skill-mcp stats` and identify top recurring skills.
2. Export raw data with `skill-mcp usage export --format json` for trend inspection.
3. Correlate search source breakdown with discovery strategy (local/catalog/github).
4. Reset outliers with `skill-mcp usage reset <name>` when testing ranking changes.
